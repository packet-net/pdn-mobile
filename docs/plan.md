...
| **App Transport Security / self-signed LAN** | `NSAllowsLocalNetworking=true` (no-justification ATS opt-out for RFC-1918/`.local`); never `NSAllowsArbitraryLoads`; TOFU/pin self-signed | `network_security_config.xml` with **scoped** `cleartextTrafficPermitted` for RFC-1918 + `pico-node.local`; `<certificates src="user"/>` or pinning | Both scoped, reviewable opt-outs — portable intent, different files. **Consult:** keep the pico AP portal plain-HTTP (local-only) so neither platform needs a cert |
